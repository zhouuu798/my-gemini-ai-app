...
if "当前步骤" not in st.session_state:
    ...
    st.session_state.步骤历史 = ["步骤一_输入资料"] # 记录浏览足迹，用于精确返回上一步
...
def 返回上一步():
    if len(st.session_state.步骤历史) > 1:
        st.session_state.步骤历史.pop() # 弹出当前页
...
if st.session_state.当前步骤 == "步骤一_输入资料":
    ...
    if st.button("🚀 开始高级创意脑暴", type="primary", use_container_width=True):
        if not 产品输入.strip() and not 上传图片列表:
            ...
            st.session_state.当前方案数量 = 3 # 重置数量
...
# --- 步骤二：分镜脚本多轮迭代 ---
    ...
    反馈意见 = st.text_input("请输入修改意见（例如：‘把第3镜头的场景换成办公室’、‘让旁白更有网感’）：")
...
# --- 步骤三：最终交付与提示词精修迭代 ---
    ...
    if not st.session_state.最终提示词输出:
        with st.spinner("🤖 提示词专家正在将分镜脚本转化为高精度 AI 视频生成模型提示词..."):
            系统提示词 = "你是一名专业的导演兼提示词工程师。请将用户确认的最终分镜脚本，严格转化为符合AI视频模型（如Sora/Runway/Pika）规范的英文/中文视觉提示词（Prompts），按镜头顺序列出。"
            用户提示词 = f"这是最终通过的分镜脚本：\n{st.session_state.当前剧本草案}"
    ...
    提示词反馈 = st.text_input("请输入对提示词的微调意见（例如：‘将所有画面风格改为赛博朋克风’、‘增加镜头推进的速度感描述’）：")
        ...
        if st.button("✨ 优化提示词", use_container_width=True):
            if not 提示词反馈.strip():
                ...
                with st.spinner("⚙️ 提示词重组工程进行中..."):
                    系统提示词 = "你是一名资深提示词专家。请根据用户最新的风格化、镜头感修改意见，对目前的AI视频提示词文本进行全局润色与重构，输出更新后的完整版提示词。"
                    st.session_state.提示词修改历史.append({"role": "user", "content": 提示词FEEDBACK := f"请根据此要求精修提示词：{提示词反馈}"})
